    ...
    try {
      ...
      if (!questionList || questionList.length === 0) {
        ...
        if (selectedStatus === 'unmastered') {
          ...
          msg = 'Bạn hiện chưa có từ vựng nào được đánh dấu "Đã thuộc" trong bộ lọc này.';
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8 space-y-6 animate-fade-in transition-colors duration-200">
        <div className="flex items-center space-x-2 text-sm text-theme-subtle font-medium">
          <button onClick={() => navigate('/')} className="flex items-center space-x-1 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors cursor-pointer">
        ...
        <div className="bg-surface border border-theme rounded-3xl p-8 text-center shadow-xl space-y-6">
          ...
          <div className="grid grid-cols-3 gap-3 py-2">
            <div className="bg-input-theme rounded-2xl p-4 border border-theme-subtle">
              ...
              <span className="text-2xl font-black text-indigo-600 dark:text-indigo-400 mt-1 block">{score} / {questions.length}</span>
            </div>
            <div className="bg-input-theme rounded-2xl p-4 border border-theme-subtle">
              <span className="text-xs text-theme-subtle font-medium block">Độ chính xác</span>
              <span className="text-2xl font-black text-emerald-600 dark:text-emerald-400 mt-1 block">{accuracy}%</span>
            </div>
            <div className="bg-input-theme rounded-2xl p-4 border border-theme-subtle">
              <span className="text-xs text-theme-subtle font-medium block">Chuỗi cao nhất</span>
              <span className="text-2xl font-black text-amber-600 dark:text-amber-400 mt-1 block">🔥 {streak}</span>
            </div>
          </div>
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-6 space-y-6 animate-fade-in transition-colors duration-200">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-3">
          {streak >= 2 && (
            <div className="flex items-center space-x-1 px-3 py-1 rounded-full bg-amber-500/15 border border-amber-500/30 text-amber-700 dark:text-amber-400 text-xs font-bold animate-pulse shadow-xs">
              <Flame className="w-3.5 h-3.5" />
              <span>Chuỗi {streak}</span>
            </div>
          )}

          <div className="px-3 py-1 rounded-full bg-indigo-500/15 border border-indigo-500/30 text-indigo-700 dark:text-indigo-400 text-xs font-extrabold shadow-xs">
            Điểm: {score}
          </div>
        </div>
      </div>
          ...
          {mode === 'fill_meaning' || mode === 'multiple_choice' ? (
            ...
            <div className="space-y-2">
              ...
              {currentQ.part_of_speech && (
                <span className="text-xs px-2.5 py-0.5 rounded-full bg-indigo-500/15 text-indigo-700 dark:text-indigo-400 font-bold border border-indigo-500/30">
                  {currentQ.part_of_speech}
                </span>
              )}
            </div>
            ...
            {currentQ.options?.map((option, idx) => {
              ...
              if (checkedResult) {
                if (option === checkedResult.expected_answer) {
                  btnClass = 'bg-emerald-500/20 border-2 border-emerald-500 text-emerald-800 dark:text-emerald-300 font-bold ring-2 ring-emerald-500/50';
                } else if (option === userAnswer && !checkedResult.is_correct) {
                  btnClass = 'bg-rose-500/20 border-2 border-rose-500 text-rose-800 dark:text-rose-300 line-through';
                } else {
            ...
            {!checkedResult && (
              <div className="flex items-center justify-between text-xs">
                <button
                  ...
                  className="text-theme-subtle hover:text-amber-600 dark:hover:text-amber-400 flex items-center gap-1 transition-colors cursor-pointer font-medium"
                >
                  <HelpCircle className="w-3.5 h-3.5" />
                  <span>Cần gợi ý?</span>
                </button>
                {showHint && (
                  <span className="text-amber-700 dark:text-amber-400 font-bold bg-amber-500/15 px-2 py-0.5 rounded border border-amber-500/30">
